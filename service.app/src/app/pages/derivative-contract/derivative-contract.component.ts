...
import { DialogDerivativeContractComponent } from "./dialog-derivative-contract/dialog-derivative-contract.component";
...
@Component({
  selector: 'ngx-derivative-contract',
  templateUrl: './derivative-contract.component.html',
  styleUrls: ['./derivative-contract.component.scss']
})
export class DerivativeContractComponent implements OnInit {
  ...
  onEdit($event) {
    this.dialogService
      .open(DialogDerivativeContractComponent, {
        closeOnEsc: false,
        ...
        context: {
          ...
          derivative_type: $event.data.derivative_type,
          hub: $event.data.hub,
          product: $event.data.product,
          lots: $event.data.lots,
          lot_size: $event.data.lot_size,
          uom: $event.data.uom,
          start_date: $event.data.start_date,
          end_date: $event.data.end_date,
          strike_price: $event.data.strike_price,
  ...
  exportAsXLSX() {
    this.source
      ...
      .then((value) => {
        this.excelService.exportAsExcelFile(value, "derivative_contracts");
      })
      .catch((e) => {
  ...
  onView($event) {
    this.dialogService.open(DialogDerivativeContractComponent, {
      closeOnEsc: false,
      closeOnBackdropClick: false,
      context: {
        title: "Edit Derivative",
  ...
  exportAsCSV() {
    this.source
      .getFilteredAndSorted()
      .then((value) => {
        this.excelService.exportToCsv(value, "derivative_contracts");
      })
      .catch((e) => {
        this.messageService.showToast("danger", "Error", e);
      });
