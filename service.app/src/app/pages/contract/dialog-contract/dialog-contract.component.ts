...
export class DialogContractComponent implements OnInit {
  ...
  operator_label: any;
  constructor(
    protected ref: NbDialogRef<DialogContractComponent>,
    ...
    this.apiContract
      ...
      .subscribe({
        next: (res) => {
          ...
          this.contractForm.controls["addendum"].setValue(res["addendum"]);
          ...
          if (res["nomination_id"] != null) {
            this.contractForm.controls["nomination_id"].setValue([
              { label: res["nomination_label"], id: res["nomination_id"] },
            ]);
          }
      ...
      .add(() => {
        ...
        if (
          ...
          this.operator_label !== undefined
        ) {
          this.contractForm.controls["operator"].setValue({
            id: this.operator_id,
            itemName: this.operator_label,
          });
          this.selectedItems = [
            { id: this.operator_id, itemName: this.operator_label },
          ];
        }
  ...
  add() {
    ...
    if (this.contractForm.valid) {
      if (
        ...
        var start_time =
          pad(this.contractForm.get("start_time").value.getHours()) +
          ":" +
          pad(this.contractForm.get("start_time").value.getMinutes());

        var end_time =
          pad(this.contractForm.get("end_time").value.getHours()) +
          ":" +
          pad(this.contractForm.get("end_time").value.getMinutes());
      } else {
        start_time = null;
        end_time = null;
      }
      var nomination_id = this.contractForm.get("nomination_id").value
        ? this.contractForm.get("nomination_id").value[0].id
        : null;
      ...
      if (this.action == "Add") {
        this.apiContract
          .addLogisticContract(this.clean(object))
          .subscribe({
            next: (res) => {
              this.submitLogisticContracts([res["id"]], true);
              this.messageService.showToast(
                "success",
                "Success",
            ...
            error: (e) => {
              if (e.error.title == "Internal Server Error") {
                this.messageService.showToast(
                  "danger",
                  "Error",
                  "Internal server error while adding logistic contract"
                );
              } else {
                this.messageService.showToast("danger", "Error", e.error);
              }
  ...
  update(object) {
    this.apiContract
      .updateLogisticContract(this.clean(object))
      .subscribe({
        next: (res) => {
          this.submitLogisticContracts([res.id], true);
          this.messageService.showToast(
        ...
        error: (e) => {
          if (e.error.title == "Internal Server Error") {
            this.messageService.showToast(
              "danger",
              "Error",
              "Internal server error while updating logistic contract"
            );
          } else {
            this.messageService.showToast("danger", "Error", e.error);
          }
  ...
  getServices(connection_type?) {
    this.loading = true;
    this.loading_array[9] = true;
    this.apiConnectionPoint
      .getLogisticServices(connection_type)
      .subscribe({
        next: (res) => {
    ...
    if (this.contractForm.get("balancing_zone").value) {
      ...
      result.map((x) => {
        ...
        this.start_time_hour =
          pad(this.start_time.getHours()) +
          ":" +
          pad(this.start_time.getMinutes());
      });
    }
  }
  ...
  getNominationById(id) {
    ...
    this.apiContract
      ...
      .add(() => {
        this.loading_array[12] = false;
        if (this.loading_array.includes(true) == false) {
          this.loading = false;
        }
      });
  }
  submitLogisticContracts(array, automatic?) {
    this.loading = true;
    this.apiJobs
      ...
      .subscribe({
        next: (res) => {
          if (automatic && res["failed_submissions"].length == 0) {
            this.ref.close("save");
          } else if (
            automatic == undefined ||
            (automatic == null && res["failed_submissions"].length == 0)
          ) {
            this.ref.close();
          }
          if (res["failed_submissions"].length == 0) {
            this.messageService.showToast("success", "Success", res["message"]);
          } else {
            var contracts = [];
        ...
        error: (e) => {
          ...
          if (e.error.title == "Internal Server Error") {
            this.messageService.showToast(
              "danger",
              "Error",
              "Internal server error while submit logistic contract"
            );
          } else {
            this.messageService.showToast("danger", "Error", e.error);
          }
  ...
  openErrorDialog(error, message, array, automatic?) {
    this.dialogService
      ...
      .onClose.subscribe((val) => {
        if (val === "yes") {
          this.submitLogisticContracts(array, automatic);
        } else {
          if (automatic) {
            this.ref.close("save");
          } else {
