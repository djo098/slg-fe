...
@Component({
  selector: "ngx-dialog-virtual-exchanges",
  templateUrl: "./dialog-virtual-exchanges.component.html",
  styleUrls: ["./dialog-virtual-exchanges.component.scss"],
})
export class DialogVirtualExchangesComponent implements OnInit {
  @Input() action: string;
  @Input() title: string;
  @Input() id: number;
  ...
  constructor(
    ...
    protected ref: NbDialogRef<DialogVirtualExchangesComponent>,
    ...
    if (this.exchangeForm.valid) {
      ...
      if (this.action == "Add") {
        this.apiThirdPartyContract
          .addThirdPartyContractVirtual("third_party_swap", this.clean(object))
          .subscribe({
            next: (res1) => {
              document
                    ...
                    .subscribe({
                      next: (res) => {
                        ...
                        this.messageService.showToast(
                          "success",
                          "Success",
                          "Virtual Swap contract added successfully in SLG!"
                        );
                      },
                      error: (e) => {
                        if (e.error.title == "Internal Server Error") {
                          this.messageService.showToast(
                            "danger",
                            "Error",
                            "Internal server error while adding gas virtual swap contract"
                          );
                        } else {
                          this.messageService.showToast(
                            "danger",
            ...
            error: (e) => {
              ...
              if (e.error.title == "Internal Server Error") {
                this.messageService.showToast(
                  "danger",
                  "Error",
                  "Internal server error while adding gas virtual swap contracts"
                );
              } else {
  ...
  update(object) {
    this.apiThirdPartyContract
      .updateThirdPartyContractVirtual("third_party_swap", object)
      .subscribe({
        next: (res) => {
          document
            .querySelector("revo-grid")
                ...
                .subscribe({
                  next: (res) => {
                    ...
                    this.messageService.showToast(
                      "success",
                      "Success",
                      "Virtual Swap contract updated successfully in SLG!"
                    );
                  },
                  error: (e) => {
                    this.loading = false;
        ...
        error: (e) => {
          ...
          if (e.error.title == "Internal Server Error") {
            this.messageService.showToast(
              "danger",
              "Error",
              "Internal server error while updating gas virtual swap contracts"
            );
          } else {
            this.messageService.showToast("danger", "Error", e.error);
          }
    ...
    this.apiThirdPartyContract
      ...
      .subscribe({
        next: (res) => {
          ...
          this.rows.map((x) => {
            x["price"] = this.numeral(x["price"]).format("0,0.[0000]");
